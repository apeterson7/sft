...
def create_augmented_dataloader(args, dataset):
    ...
    # Sample 5000 random examples from the training set and apply transformation
    indices = random.sample(range(len(dataset["train"])), 5000)
    small_train = dataset["train"].select(indices)
    transformed = small_train.map(custom_transform, load_from_cache_file=False)
    ...
    # Concatenate original tokenized training data with transformed examples
    combined = datasets.concatenate_datasets([tokenized_dataset["train"], transformed_tokenized])
